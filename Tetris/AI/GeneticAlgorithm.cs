...
namespace Tetris
{
    public static class GeneticAlgorithm
    {
        ...
        private static readonly List<GeneticTetris> Players = new List<GeneticTetris>();
        ...
        public static async Task AlgorithmStart(Form1 form1 = null)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                for (int i = 0; i < 8; i++)
                {
                    var player = new GeneticTetris(Form1, 1 + (i % 4 * 11), 15 + (i / 4 * 21),
                        KeyboardPlayer1.GetInstance, i + 1);
                    PlayerAdd(player);
                    TetrisAI.AutoPlay(player);
                }
